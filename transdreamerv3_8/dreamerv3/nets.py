...
class RSSM(nj.Module):
  ...
  def img_step(self, prev_state, prev_action):
    ...
    x, deter = self.get('transformer', Transformer, d_model_inner=64,
        num_heads=8, feed_forward_dim=self._deter, num_layers=6, 
            kw={**self._kw, 'act':'none'})(x, prev_state['deter'])
...
class MultiHeadSelfAttention(nj.Module):
  ...
  def __call__(self, x):
    ...
    query = self.get('linear1_M', Linear, **{'units':self.d_model_inner * self.num_heads})(x)
    query = query.reshape(batch_size, -1, self.num_heads, self.head_dim)
    key = self.get('linear2_M', Linear, **{'units':self.d_model_inner*self.num_heads})(x)
    key = key.reshape(batch_size, -1, self.num_heads, self.head_dim)
    value = self.get('linear3_M', Linear, **{'units':self.d_model_inner*self.num_heads})(x)
    ...
    attention_output = jnp.einsum('bhqk,bkhd->bqhd', weights, value).reshape(batch_size, -1, 
                          self.d_model_inner*self.num_heads)

    # Final dense layer
    output = self.get('linear4_M', Linear, **{'units':embed_dim})(attention_output)
...
class TransformerLayer(nj.Module):
  ...
  def __call__(self, x):
    ...
    attn_output = self.get('MultiHeadSelfAttention',MultiHeadSelfAttention, 
            self.d_model_inner, self.num_heads, self._kw)(x)

    attn_output = self.get('layernorm1_TL', Norm, 'layer')(x + attn_output)

    # Feed-forward
    ff_output = self.get('linear1_TL', Linear, **{'units':self.feed_forward_dim})(attn_output)
    ff_output = jax.nn.relu(ff_output)
    ff_output = self.get('linear2_TL', Linear, **{'units':embed_dim})(ff_output)
    ff_output = self.get('layernorm2_TL', Norm, 'layer')(attn_output + ff_output)
...
class Transformer(nj.Module):
  ...
  def __call__(self, x, deter):
    ...
    for _ in range(self.num_layers):
      x = self.get('TransformerLayer', TransformerLayer, self.d_model_inner, 
            self.num_heads, self.feed_forward_dim, self._kw)(x)

    # Add an additional Dense layer to project down to the desired size
    x = self.get('linear2_T', Linear, **{'units':deter.shape[1]})(x)
